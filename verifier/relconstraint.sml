...
structure RelConstraint = 
    ...
    type env = Le.t * RLe.t * guard_t 
		...
		datatype refinement_constraint =
		    RSubRef of RLe.t * guard_t * RF.refinement * RF.refinement * bool * int * (Con.t option) * int * (subref_id option) 
		  | RWFRef of RLe.t * RF.refinement * (subref_id option) 
		...
		fun pprint_ref so refcons =
			case refcons of 
		    	  RSubRef (env,g,r1,r2,_,_,_,_,io) =>
		...
		*)		    		
		...
		fun make_lc c fc funflag recordIndex con conrecordIndex = case c of 
			  lc c' => (lc {lc_cstr = fc, lc_orig = RCstr c, lc_id = #lc_id c', lc_binding = #lc_binding c'}, funflag, recordIndex, con, conrecordIndex)
			  		
		fun lequate_cs env g c funflag recordIndex con conrecordIndex variance (f1, f2) = 
			case variance of
		  		  RF.RInvariant => [make_lc c (RSubFrame(env,g,f1,f2)) funflag recordIndex con conrecordIndex, 
		  		  					make_lc c (RSubFrame(env,g,f2,f1)) funflag recordIndex con conrecordIndex]
		  		| RF.RCovariant => [make_lc c (RSubFrame(env,g,f1,f2)) funflag recordIndex con conrecordIndex]
		  		| RF.RContravariant => [make_lc c (RSubFrame(env,g,f2,f1)) funflag recordIndex con conrecordIndex]
		
		(*
      fun match_and_extend env (l1,f1) (l2,f2) =
			case (l1,l2) of
		  		  (SOME p, NONE) => RLe.env_bind env p f2
		  		| (NONE, SOME p) => RLe.env_bind env p f2
		  		| (SOME p1, SOME p2) =>
		  			if Pattern.same (p1, p2) then RLe.env_bind env p1 f2 else env
		  		| _  => env
     *)
